...
{
	if(!plen) return NULL;
	return Obj_getHexString(m_obj, plen);
}
...
{
	return Page_findGetEndChar( m_handle, find_index );
}
-(void)dealloc
{
...
-(int)getLineStyle
{
    return Page_getAnnotLineStyle(m_page, m_handle);
}
...
{
	return Page_getAnnotStrokeDash(m_page, m_handle, dashs, dashs_max);
}
...
{
	return Page_getAnnotEditTextAlign( m_page, m_handle );
}
-(bool)setEditTextAlign:(int)align
{
	return Page_setAnnotEditTextAlign( m_page, m_handle, align );
}
...
-(bool)flateFromPage
{
	bool ret = Page_flateAnnot( m_page, m_handle );
	m_handle = NULL;
	m_page = NULL;
	return ret;
}
...
{
	return Document_getLinearizedStatus(m_doc);
}
...
-(int)getJSCount
{
	return Document_getJSCount(m_doc);
}
-(NSString *)getJSName:(int)idx
{
	return Document_getJSName(m_doc, idx);
}
-(NSString *)getJS:(int)idx
{
	return Document_getJS(m_doc, idx);
}
...
-(NSString *)getXMP
{
	return Document_getXMP(m_doc);
}
-(bool)setXMP:(NSString *)xmp
{
    return Document_setXMP(m_doc,xmp);
}
-(bool)save
{
...
{
    return Document_getMeta(m_doc, [tag UTF8String]);
}
